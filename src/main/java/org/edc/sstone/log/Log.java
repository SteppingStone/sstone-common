...
/**
 * Maintains a recording of the application's operation. The <code>Log</code> takes in strings for
 * events that happen during the program's execution. Each event has a <code>Level</code>, or
 * priority, associated with it.
 * <p>
 * The log can be read later to examine problems.
 * <p>
 * The following two examples illustrate how to record things in the log. The first statement shows
 * that the logging level should be checked before logging because it stops the expensive string
 * concatenation when the log level is off. The second shows logging an exception.
 * 
 * <code><pre>
...
public class Log {
    /**
     * The number of log messages maintained. The log store is a circular buffer of this size. This
     * number should be great enough that if a series of problems occur the conditions leading up to
     * them can be seen. Conversely it should be small enough to not take too much memory (if 10
     * messages are stored averaging 50 characters at 2 bytes per character that would take up 1 KB
     * of heap space).
     */
    private static final int MAX_LOG_MESSAGES = 25;

    /**
     * The log message store. All log messages are recorded in this circular buffer. Once
     * <code>MAX_LOG_MESSAGES</code> more messages are logged the message will be discarded.
     */
     ...
     * 
     * @return The value of the lowest level of log statements written to the log. It can later be
     *         passed to the <code>setLevel</code> method to reset the logging level.
     */
    ...
    /**
     * Sets the level log statements are evaluated. Anything at <code>level</code> or higher will be
     * logged.
     * 
     * @param level
     ...
     * level of this category with {@link Level#DEBUG DEBUG} Level. If the category is
     * <code>DEBUG</code> enabled, then it will log the message.
     * 
     * @param message
    ...
    /**
     * Log a message string with the <code>DEBUG</code> level including the stack trace of the
     ...
     * <p>
     * This method first checks if this category is <code>INFO</code> enabled by comparing the level
     * of this category with {@link Level#INFO INFO} Level. If the category is <code>INFO</code>
    ...
    /**
     * Log a message string with the <code>INFO</code> level including the stack trace of the
     * {@link Throwable} <code>t</code> passed as parameter.
     * <p>
     ...
     * <p>
     * This method first checks if this category is <code>WARN</code> enabled by comparing the level
     * of this category with {@link Level#WARN WARN} Level. If the category is <code>WARN</code>
     * enabled, then it will log the message.
     * 
     * @param message
     *            is the message to log.
    ...
    /**
     * Log a message string with the <code>WARN</code> level including the stack trace of the
     * {@link Throwable} <code>t</code> passed as parameter.
     * <p>
     ...
     * level of this category with {@link Level#ERROR ERROR} Level. If the category is
     * <code>ERROR</code> enabled, then it will log the message.
     * 
     * @param message
     *            is the message to log.
     */
    ...
    /**
     * Log a message string with the <code>ERROR</code> level including the stack trace of the
     * {@link Throwable} <code>t</code> passed as parameter.
     * <p>
     ...
     * <p>
     * This function is intended to lessen the computational cost of disabled log statements. All
     * debug logs that perform string concatenation should be written as:
     * 
     * <pre>
     * if (Log.isDebugEnabled())
     * {
     ...
     * <p>
     * This function is intended to lessen the computational cost of disabled log statements. All
     * info logs that perform string concatenation should be written as:
     * 
     * <pre>
     ...
     * <p>
     * The log store is a circular buffer. This method maintains it. Once the maximum size is
     * reached, the oldest message logged will be replaced with this one.
     * 
     * @param message
     ...
     * @param throwable
     *            is an exception that caused the log message. This will be <code>null</code> if no
     *            exception caused the message.
     */
    ...
    /**
     * Gets all the log messages still in memory. Internally the log messages are kept in a circular
     * buffer and once it fills, the oldest messages will be discarded.
     * <p>
     * The returned array references all of the log messages and does not stop logging from
     * continuing. In other words the returned logs are a snapshot in time.
     * 
     * @return An array of the previously logged messages. The higher the array index, the more
     *         recently it was logged. Therefore <code>length - 1</code> will be the last message
     *         logged. If no messages have been logged this will return an array of length zero
     *         (i.e. it never returns <code>null</code>).
     */
